...
import com.webdesign488.post.MainActivity;
import com.webdesign488.post.SpendActivity;
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			if(AppManager.isAppmanager()){
				Intent i = new Intent(context, SpendActivity.class);
				i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				context.startActivity(i);
			}else{
/*				Intent i = new Intent(context,  AppManager.getAppManager().currentActivity().getClass());
				i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				context.startActivity(i);
*/
            // context.startActivity(new Intent(context, AppManager.getAppManager().currentActivity().getClass()));
			}
			
		/*	Log.d(TAG, "[MyReceiver] 用户点击打开了通知");
			if (!AppManager.isAppmanager()) {
				// 打开自定义的Activity
				Intent i = new Intent(context, ChatInfoActivity.class);
				i.putExtras(bundle);
				i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				// i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
	 ...
	 * (MainActivity.isForeground) { String message =
