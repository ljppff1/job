...
import com.webdesign488.post.util.SharedPreferenceDB8;
import com.webdesign488.post.util.SharedPreferenceDB8;
import com.webdesign488.post.util.Util;
import com.webdesign488.post.view.ProgressDialog;

public class ChoiceArea5Activity extends BaseActivity {
	...
	private void initData() {
	     ...
	     list.add("locationoneid");
	     List<String> list1 =new ArrayList<String>();
	     list1.add(ID);
   AsyncTaskUtil.getJson(list, list1,handler, dialog, Content.AREA2, ChoiceArea5Activity.this, 3);
						...
						try {
							...
							if(code ==1){
	                            ...
	                            for(int i=0;i<data.length();i++){
	                            	Integer areaId = data.getJSONObject(i).getInt("locationtwoid");
	        						String areaName =  data.getJSONObject(i).getString("locationtwoname");
	        						Area area =new Area();
	        						area.setAreaId(areaId+"");
	        						area.setAreaName(areaName);
	                                if(SharedPreferenceDB8.isChecked(areaId+"", ChoiceArea5Activity.this)){
		...
		mLvArea1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
			 ...
			 if(area1List.get(position).getIsChecked()){
				 ...
				 SharedPreferenceDB8.deleteSp(area1List.get(position).getAreaId(),ChoiceArea5Activity.this);
			 }else{
				 area1List.get(position).setIsChecked(true);
				 SharedPreferenceDB8.addSp(area1List.get(position).getAreaId(),ChoiceArea5Activity.this,area1List.get(position).getAreaName());
			 }
				adapter.notifyDataSetChanged();
			}

		});
		dialog =new com.webdesign488.post.view.ProgressDialog(ChoiceArea5Activity.this);
	}
	OnClickListener listener =new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.mArea3sure:
				AppManager.getAppManager().finishActivityJob4();
				break;
				...
				if(mArea3SelectAll.getText().equals(getResources().getString(R.string.selectall))){
					...
					for(int i=0;i<area1List.size();i++){
						 if(!area1List.get(i).getIsChecked()){
							 ...
							 SharedPreferenceDB8.addSp(area1List.get(i).getAreaId(),ChoiceArea5Activity.this,area1List.get(i).getAreaName());
						 }
					}
					adapter.notifyDataSetChanged();
				}else{
					...
					for(int i=0;i<area1List.size();i++){
					 if(area1List.get(i).getIsChecked()){
						 ...
						 SharedPreferenceDB8.deleteSp(area1List.get(i).getAreaId(),ChoiceArea5Activity.this);
					 }
				}
					adapter.notifyDataSetChanged();
				}
