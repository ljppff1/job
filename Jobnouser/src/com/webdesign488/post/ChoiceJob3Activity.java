...
import com.webdesign488.post.util.SharedPreferenceDB3;
import com.webdesign488.post.util.SharedPreferenceDB3;
import com.webdesign488.post.util.Util;
import com.webdesign488.post.view.ProgressDialog;

public class ChoiceJob3Activity extends BaseActivity {
	...
	private void initData() {
	     ...
	     list1.add(ID);
  AsyncTaskUtil.getJson(list, list1,handler, dialog, Content.JOB3, ChoiceJob3Activity.this, 3);
	}
						...
						try {
							...
							if(code ==1){
	                            ...
	                            for(int i=0;i<data.length();i++){
	                            	Integer areaId = data.getJSONObject(i).getInt("hid");
	        						String areaName =  data.getJSONObject(i).getString("hname");
	        						Area area =new Area();
	        						area.setAreaId(areaId+"");
	        						area.setAreaName(areaName);
	                                if(SharedPreferenceDB3.isChecked(areaId+"", ChoiceJob3Activity.this)){
		...
		mLvArea1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
			 ...
			 if(area1List.get(position).getIsChecked()){
				 ...
				 SharedPreferenceDB3.deleteSp(area1List.get(position).getAreaId(),ChoiceJob3Activity.this);
			 }else{
				 area1List.get(position).setIsChecked(true);
				 SharedPreferenceDB3.addSp(area1List.get(position).getAreaId(),ChoiceJob3Activity.this,area1List.get(position).getAreaName());
			 }
				adapter.notifyDataSetChanged();
			}

		});
		dialog =new com.webdesign488.post.view.ProgressDialog(ChoiceJob3Activity.this);
	}
				...
				if(mArea3SelectAll.getText().equals(getResources().getString(R.string.selectall))){
					...
					for(int i=0;i<area1List.size();i++){
						 if(!area1List.get(i).getIsChecked()){
							 ...
							 SharedPreferenceDB3.addSp(area1List.get(i).getAreaId(),ChoiceJob3Activity.this,area1List.get(i).getAreaName());
						 }
					}
					adapter.notifyDataSetChanged();
				}else{
					...
					for(int i=0;i<area1List.size();i++){
					 if(area1List.get(i).getIsChecked()){
						 ...
						 SharedPreferenceDB3.deleteSp(area1List.get(i).getAreaId(),ChoiceJob3Activity.this);
					 }
				}
					adapter.notifyDataSetChanged();
				}
