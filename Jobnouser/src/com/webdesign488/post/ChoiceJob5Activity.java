...
import com.webdesign488.post.util.SharedPreferenceDB4;
import com.webdesign488.post.util.SharedPreferenceDB4;
import com.webdesign488.post.util.Util;
import com.webdesign488.post.view.ProgressDialog;

public class ChoiceJob5Activity extends BaseActivity {
	...
	class MyTask extends AsyncTask<List<NameValuePair>, Integer, String> {
		...
		protected String doInBackground(List<NameValuePair>... params) {
			...
			String json = null;
			try {
				json = sdClient.post(Content.JOB2, params[0]);
			} catch (Exception e) {
					...
					try {
						...
						if (code == 1) {
							...
							for (int i = 0; i < data.length(); i++) {
								...
								if (SharedPreferenceDB4.isChecked(areaId + "",
										ChoiceJob5Activity.this)) {
		...
		mLvArea1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (area1List.get(position).getIsChecked()) {
					...
					SharedPreferenceDB4.deleteSp(area1List.get(position)
							.getAreaId(), ChoiceJob5Activity.this);
				} else {
					area1List.get(position).setIsChecked(true);
					SharedPreferenceDB4.addSp(area1List.get(position)
							.getAreaId(), ChoiceJob5Activity.this, area1List
				...
				if(mArea3SelectAll.getText().equals(getResources().getString(R.string.selectall))){
					...
					for(int i=0;i<area1List.size();i++){
						 if(!area1List.get(i).getIsChecked()){
							 ...
							 SharedPreferenceDB4.addSp(area1List.get(i).getAreaId(),ChoiceJob5Activity.this,area1List.get(i).getAreaName());
						 }
					}
					adapter.notifyDataSetChanged();
				}else{
					...
					for(int i=0;i<area1List.size();i++){
					 if(area1List.get(i).getIsChecked()){
						 ...
						 SharedPreferenceDB4.deleteSp(area1List.get(i).getAreaId(),ChoiceJob5Activity.this);
					 }
				}
					adapter.notifyDataSetChanged();
				}
