...
import com.webdesign488.post.util.SharedPreferenceDB;
import com.webdesign488.post.util.SharedPreferenceDB;
					...
					try {
						...
						if (code == 1) {
							...
							for (int i = 0; i < data.length(); i++) {
								...
								if (SharedPreferenceDB.isChecked(areaId + "",
		...
		mLvArea1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (area1List.get(position).getIsChecked()) {
					...
					SharedPreferenceDB.deleteSp(area1List.get(position)
							.getAreaId(), ChoiceArea3Activity.this);
				} else {
					area1List.get(position).setIsChecked(true);
					SharedPreferenceDB.addSp(area1List.get(position)
				...
				if(mArea3SelectAll.getText().equals(getResources().getString(R.string.selectall))){
					...
					for(int i=0;i<area1List.size();i++){
						 if(!area1List.get(i).getIsChecked()){
							 ...
							 SharedPreferenceDB.addSp(area1List.get(i).getAreaId(),ChoiceArea3Activity.this,area1List.get(i).getAreaName());
						 }
					}
					adapter.notifyDataSetChanged();
				}else{
					...
					for(int i=0;i<area1List.size();i++){
					 if(area1List.get(i).getIsChecked()){
						 ...
						 SharedPreferenceDB.deleteSp(area1List.get(i).getAreaId(),ChoiceArea3Activity.this);
					 }
				}
					adapter.notifyDataSetChanged();
				}
