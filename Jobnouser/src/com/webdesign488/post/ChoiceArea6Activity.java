...
import com.webdesign488.post.util.SharedPreferenceDB2;
import com.webdesign488.post.util.SharedPreferenceDB2;
					...
					try {
						...
						if (code == 1) {
							...
							for (int i = 0; i < data.length(); i++) {
								...
								if (SharedPreferenceDB2.isChecked(areaId + "",
		...
		mLvArea1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (area1List.get(position).getIsChecked()) {
					...
					SharedPreferenceDB2.deleteSp(area1List.get(position)
							.getAreaId(), ChoiceArea6Activity.this);
				} else {
					area1List.get(position).setIsChecked(true);
					SharedPreferenceDB2.addSp(area1List.get(position)
				...
				if(mArea3SelectAll.getText().equals(getResources().getString(R.string.selectall))){
					...
					for(int i=0;i<area1List.size();i++){
						 if(!area1List.get(i).getIsChecked()){
							 ...
							 SharedPreferenceDB2.addSp(area1List.get(i).getAreaId(),ChoiceArea6Activity.this,area1List.get(i).getAreaName());
						 }
					}
					adapter.notifyDataSetChanged();
				}else{
					...
					for(int i=0;i<area1List.size();i++){
					 if(area1List.get(i).getIsChecked()){
						 ...
						 SharedPreferenceDB2.deleteSp(area1List.get(i).getAreaId(),ChoiceArea6Activity.this);
					 }
				}
					adapter.notifyDataSetChanged();
				}
